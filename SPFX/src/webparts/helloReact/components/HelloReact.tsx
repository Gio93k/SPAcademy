...
export default class HelloReact extends React.Component<IHelloReactProps, IHelloReactState> {
  ...
  public async componentDidMount() {
    ...
    const groups = await GraphHelper.getUserGroups();
    this.setState({ groupstate: groups.value });
  }
  ...
  public render(): React.ReactElement<IHelloReactProps> {
    return (
      <div>
        ...
        <br />
        <label> Gruppi:</label>
        {this.state.groupstate != undefined ?
          //  <DetailsList items={this.state.groupstate.map((val) => { return { text: val.displayName } })} />
          <DetailsList items={this.state.groupstate.map((val) => { return { NomeGruppo: val.displayName } })} />
          : <p> Loading gruppo.... </p>}
